...
quatro dimensões de avaliação. O coeficiente de determinação MZ foi de $R^2 = 0{,}667$
para as dollar bars, contra $R^2 = 0{,}493$ para as time bars, diferença de 17,4 pontos
percentuais.
...
interceptos negativos da regressão MZ ($\hat{\alpha}_{\text{DB}} = -9{,}20 \times 10^{-5}$ e $\hat{\alpha}_{\text{TB}} = -2{,}54 \times 10^{-4}$). Esse padrão é característico de
...
significativa: redução de 36\% no MSE, de 26\% no MAE e de 17 pontos percentuais no
$R^2$ da regressão de Mincer-Zarnowitz, além de ICs bootstrap para o QLIKE que não
se sobrepõem a 95\% de confiança.

A significância estatística formal, avaliada pelo teste de Diebold-Mariano, atinge o limiar convencional de 1\% (estatística DM = $-4{,}806$, p-valor $< 0{,}001$). 
O poder do teste é limitado pelo número de dias de sobreposição entre os conjuntos de 
previsão (147 dias), mas a combinação de sinal inequívoco na estatística DM, domínio 
em todas as métricas pontuais e não sobreposição dos ICs bootstrap para o QLIKE 
configura evidência consistente e conclusiva, ao nível convencional, em favor das dollar bars.
